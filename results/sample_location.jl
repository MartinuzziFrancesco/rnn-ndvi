...
for location in locations
    ...
    for (midx,model) in enumerate(models)
        ...
        lentime = length(tempo)
        slice_dates = range(1, lentime, step=lentime ÷ 5)
        lines!(ax, 1:lentime, reduce(vcat,mean(values(ta_prediction), dims=2)),
            label="$mm",
            linewidth=2.0)
        #band!(ax, 1:lentime,
        #    reduce(vcat,mean(values(ta_prediction), dims=2)).-reduce(vcat,std(values(ta_prediction), dims=2)),
        #    reduce(vcat,mean(values(ta_prediction), dims=2)).+reduce(vcat,std(values(ta_prediction), dims=2));
        #    )
    ...
    save("./$location/full_results_mean.png", fig, px_per_unit = 2)
end
...
slice_dates_full = range(1, lentime_full, step=lentime_full ÷ 10)

tempo_train = string.(Dates.year.(timestamp(ta_gt_train)))
lentime_train = length(tempo_train)
slice_dates_train = range(1, lentime_train, step=lentime_train ÷ 7)

tempo_pred = string.(Dates.year.(timestamp(ta_gt_pred)))
lentime_pred = length(tempo_pred)
slice_dates_pred = range(1, lentime_pred, step=lentime_pred ÷ 5)

tempo_ext = string.(Dates.year.(timestamp(ta_pred_extremes)))
lentime_ext = length(tempo_ext)
slice_dates_pred = range(1, lentime_pred, step=lentime_pred ÷ 6)
...
hlines!(ulax, [sigma_neg, sigma_pos], xmax = [1, 1],
    ...
    linestyle = :dash,
    color=:firebrick3)
ulax.xticks = (1:730:5110, string.(2000:2:2012))
ulax.yticks = ([-2.0, sigma_neg, 0.0, sigma_pos, 2.0], ["-2", L"\kappa_-", "0", L"\kappa_+", "2"])

#llax.xticklabelrotation = π / 4
#llax.xticklabelalign = (:right, :center)
#=
rlax = Axis(llg[1,2],
        #title = "$location",
        #xlabel = "Years",
...
#urlax.yticks = -2.0:2.0:2.0
fig
lrlax = Axis(lowrightlowf[1,1],
    #title = "$location",
    xlabel = "Years",
    #ylabel = "Normalized NDVI",
    xticklabelsize = 32,
...
lines!(lrlax, 1:lentime_pred, reduce(vcat,mean(values(ta_gt_pred), dims=2)),
    ...
    linewidth=3.0,
    color=colors[3])
#scatter!(lrlax, Int.(ex_idx), reduce(vcat,mean(values(ta_pred_extremes), dims=2)),
#    markerwidth=2.0,
#    color=:red)
...
#lrlax.xticklabelrotation = π / 4
#lrlax.xticklabelalign = (:right, :center)
#hidedecorations!(urlax, grid = false)
rowgap!(rightlowf, 10)
